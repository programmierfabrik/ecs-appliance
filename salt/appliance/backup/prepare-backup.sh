prepare_backup () {
    ...
    echo "$APPLIANCE_BACKUP_ENCRYPT" | /opt/gpg/bin/gpg --homedir /root/.gnupg --batch --yes --import --
    # write out backup target and gpg_key to duply config
    gpg_key_id=$(/opt/gpg/bin/gpg --keyid-format 0xshort --list-key ecs_backup | grep pub | sed -r "s/pub.+0x([0-9A-F]+).+/\1/g")
